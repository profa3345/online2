...
(function (global) {
  ...
  async function exportarPDF(r) {
    try {
      ...
      if (temImgs || temVids) {
        var lm = [];
        if (r.imgsEquip    && r.imgsEquip.length)    lm.push(['Fotos de equipamento',     r.imgsEquip.length    + ' anexada(s)']);
        if (r.imgsServico  && r.imgsServico.length)  lm.push(['Fotos do serviço',          r.imgsServico.length  + ' anexada(s)']);
        if (r.imgsBaterias && r.imgsBaterias.length) lm.push(['Fotos de baterias/painel',  r.imgsBaterias.length + ' anexada(s)']);
